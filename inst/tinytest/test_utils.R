...
expect_equal(colnames(statgenQTLxT:::dfBind(list(df1, df1))), c("a", "b"))
expect_equal(colnames(statgenQTLxT:::dfBind(list(df1, df2))),
             c("a", "b", "c"))
expect_equal(colnames(statgenQTLxT:::dfBind(list(df1, df3))),
             c("a", "b", "c", "d"))
expect_equal(colnames(statgenQTLxT:::dfBind(list(df1, df2, df3))),
             c("a", "b", "c", "d"))

# Test that NAs are inserted for missing columns.
expect_equivalent(unlist(statgenQTLxT:::dfBind(list(df1, df2))),
                  c(1, 2, 1, 2, 1, 2, NA, NA, NA, NA, 1, 2))
expect_equivalent(unlist(statgenQTLxT:::dfBind(list(df1, df2, df1))),
                  c(1, 2, 1, 2, 1,2, 1, 2, NA, NA, 1, 2, NA, NA, 1, 2, NA, NA))

# Test that empty data.frames are ignored when binding.

expect_equal(statgenQTLxT:::dfBind(list(data.frame(), df1)), df1)
...
M2 <- statgenQTLxT:::nearestPD(M, corr = TRUE)
...
expect_warning(statgenQTLxT:::nearestPD(M, keepDiag = TRUE),
               "did not converge in 100 iterations")
M3 <- statgenQTLxT:::nearestPD(M, keepDiag = TRUE, maxIter = 120)
...
M4 <- statgenQTLxT:::nearestPD(M, do2eigen = FALSE)
...
M5 <- statgenQTLxT:::nearestPD(M, doDykstra = FALSE)
expect_equal(as.numeric(M5),
             c(0.604727709553899, 1.04559967845123, 0.586415044934283,
               1.04559967845123, 1.82792523675371, 0.890017983604708,
               0.586415044934283, 0.890017983604708, 1.33494200875219))

M6 <- statgenQTLxT:::nearestPD(M, doSym = TRUE)

...
#data.folder    <- 'D:/willem/Dropbox/research/statistical_genetics/new_analysis_drops/'
data.folder    <- 'D:/willem/Dropbox/research/statistical_genetics/newdata/'

# here are the R-scripts
script.folder  <- "D:/willem/Dropbox/research/statistical_genetics/multi_trait_scripts/"

# Results will be written to this folder (needs to exist!)
# should have an / at the end
results.folder <- 'F:/results'
...
r.image <- 'D:/willem/statistical_genetics_large_files/arabidopsis_data/LFNdata/LFN349acc_000_LDAK.RData' # LFN350acc_000_new_gene_annotation_ibd_kinship.RData" #LFN350acc_000.RData"
...
#pheno.file = pheno.files[1]
  ...
  write.csv(Y.unstandardized,file=paste0(trait.name,suffix,'_imputed_unstandardized.csv'),quote=F)

  varcomp.file <- paste0('varcomp_',trait.name,'_',m.G,'_',m.E,suffix,'.RData')


  if (cov.model==3) {
    gwas.file <- paste0('gwas_',trait.name,'_',m.G,'_',m.E,suffix,'.RData')
    gwas.csv  <- paste0('gwas_effects_',trait.name,'_',m.G,'_',m.E,suffix,'.csv')
    gwas.csv.t<- paste0('gwas_Tstat_',trait.name,'_',m.G,'_',m.E,suffix,'.csv')
  } else {
    gwas.file <- paste0('gwas_',trait.name,'_',suffix,'.RData')
    gwas.csv  <- paste0('gwas_effects_',trait.name,'_',suffix,'.csv')
    gwas.csv.t<- paste0('gwas_Tstat_',trait.name,'_',suffix,'.csv')
  }
  ############################################################
  ...
  if (run.gwas) {
    ...
    save(M,Tstat,results,results.wald,file=paste0(results.folder,gwas.file))
  ...
  if (cov.model==3) {
    ...
    plot.file.name      <- paste0(trait.name,'_',m.G,'_',m.E,suffix,'_manhattan.jpeg')
    plot.file.name.wald <- paste0(trait.name,'_',m.G,'_',m.E,suffix,'_manhattan_wald.jpeg')

    QQ.file.name        <- paste0(trait.name,'_',m.G,'_',m.E,suffix,'_QQplot.jpeg')
    QQ.file.name.wald   <- paste0(trait.name,'_',m.G,'_',m.E,suffix,'_QQplot_wald.jpeg')

  } else {

    QQ.file.name        <- paste0(trait.name,'_',suffix,'_QQplot.jpeg')
    QQ.file.name.wald   <- paste0(trait.name,'_',suffix,'_QQplot_wald.jpeg')

    plot.file.name      <- paste0(trait.name,'_',suffix,'_manhattan.jpeg')
    plot.file.name.wald <- paste0(trait.name,'_',suffix,'_manhattan_wald.jpeg')

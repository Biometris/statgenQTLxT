...
#
#    ## fixed.formula <- paste0('pheno ~ ', paste(names(x)[covariates], collapse ='+')) #commented out because of "x"
#
#   } else {
#
...
#
#     reml.obj <- asreml::asreml(fixed    = eval(ft),
#                        random   = ~ giv(genotype),
#                        data     = d.red,
#                        maxit    =30,
#                        workspace=2e+08,
...
#   if (length(covariates) > 0) {
#
#     ##fixed.formula <- paste0('pheno ~ trait.name + ', paste(names(x)[covariates],collapse ='+')) #commented out because of "x"
#
#   } else {
#
...
#
#         reml.obj <- asreml::asreml(fixed = pheno ~ trait.name,
#                            random = ~ us(trait.name):giv(genotype),
#                            ginverse = list(genotype = solve(K)),
#                            rcov = ~ us(trait.name):id(genotype),
#                            data = d.red,

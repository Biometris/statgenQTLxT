LL.REML.hess.diag <- function(Y,X,Dk,V.inv.array,Q.G.output,Q.E.output,
  ...
  for (g2 in 1:Q) {
    for (g1 in 1:g2) {
      ...
      # extra for REML
      prod.mat.T.extra      <- sapply(1:n,function(m){(as.numeric(V.inv.array[m,jp,i] * matrix(V.inv.array[m,,ip]) %*% t(matrix(V.inv.array[m,j,])))+as.numeric(V.inv.array[m,jp,j] * matrix(V.inv.array[m,,ip]) %*% t(matrix(V.inv.array[m,i,])))+as.numeric(V.inv.array[m,ip,i] * matrix(V.inv.array[m,,jp]) %*% t(matrix(V.inv.array[m,j,])))+as.numeric(V.inv.array[m,ip,j] * matrix(V.inv.array[m,,jp]) %*% t(matrix(V.inv.array[m,i,])))) })
      prod.mat.T.extra      <- array(t(prod.mat.T.extra),dim=c(n,p,p))
      Q.EE.ip.jp.i.j.matrix <- sapply(1:n,function(m){kronecker(matrix(X[,m]) %*% t(matrix(X[,m])),prod.mat.T.extra[m,,])})
      Q.EE.ip.jp.i.j        <- matrix(apply(Q.EE.ip.jp.i.j.matrix,1,sum),ncol=p)
      ...
      # extra for REML
      Q.GG.ip.jp.i.j            <- matrix(apply(t(dk2 * t(Q.EE.ip.jp.i.j.matrix)),1,sum),ncol=p)
      ...
      # extra for REML
      Q.GE.ip.jp.i.j            <- matrix(apply(t(dk * t(Q.EE.ip.jp.i.j.matrix)),1,sum),ncol=p)
      ...
      # extra for REML:
      E.trace            <- E.trace - sum(diag(Qinv %*% Q.EE.i.j.ip.jp))
      E.trace            <- E.trace - sum(diag(Qinv %*% Q.EE.ip.jp.i.j))
      E.trace            <- E.trace + sum(diag(Qinv %*% (Q.E.output[i,j,,] + Q.E.output[j,i,,]) %*% Qinv %*% (Q.E.output[ip,jp,,] + Q.E.output[jp,ip,,])))
      ...
      # extra for REML:
      G.trace            <- G.trace - sum(diag(Qinv %*% Q.GG.i.j.ip.jp))
      G.trace            <- G.trace - sum(diag(Qinv %*% Q.GG.ip.jp.i.j))
      G.trace            <- G.trace + sum(diag(Qinv %*% (Q.G.output[i,j,,] + Q.G.output[j,i,,]) %*% Qinv %*% (Q.G.output[ip,jp,,] + Q.G.output[jp,ip,,])))
      ...
      # extra for REML:
      GE.trace            <- GE.trace - sum(diag(Qinv %*% Q.GE.i.j.ip.jp))
      GE.trace            <- GE.trace - sum(diag(Qinv %*% Q.GE.ip.jp.i.j))
      GE.trace            <- GE.trace + sum(diag(Qinv %*% (Q.G.output[i,j,,] + Q.G.output[j,i,,]) %*% Qinv %*% (Q.E.output[ip,jp,,] + Q.E.output[jp,ip,,])))

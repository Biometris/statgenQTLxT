...
matrixPlot <- function(effectDat,
  ...
  parEffData <- data.table::data.table(merge(effectDat,
                                             map[, c("snp", "cumPos")]))
  ## Add 5 to cumPos since geom_tile uses center of tile and has width 10.
  parEffData$cumPos <- parEffData$cumPos + 5
  ## Melt all parent effects to 1 data.frame.
  parEffData <- data.table::melt(parEffData, id.vars = c("snp", "cumPos"),
                                 measure.vars = founders,
                                 variable.name = "parent",
                                 value.name = "effect")
  ## Only plotting the effects for significant SNPs. Set all others to NA.
  parEffData$effect[!parEffData$snp %in% signSnp$snp] <- NA
  p <- ggplot2::ggplot(data = parEffData,
                       ggplot2::aes_string(x = "cumPos", y = "parent",
                                           fill = "effect")) +
    ggplot2::geom_tile(ggplot2::aes(height = 1, width = 10)) +
    ggplot2::scale_fill_gradient2(low = "dodgerblue", high = "red",

LL.REML.grad.diag <- function(Y,X,Dk,V.inv.array) {
  ...
  for (j in 1:p) {
      ...
      # i=1;j=2
        ...
        if (i==j) {
          ...
          q.scal.E.j.i     <- sum(q.scal.E.j.i.vec)

          # Zhou and Stephens 2014, suppl, p.21, (42-44)
          Q.G.j.i     <- matrix(apply(t(t(Q.E.j.i.matrix) * diag(Dk)),1,sum),ncol=p)
        ...
        #################
        # so far, we have calculated the log-lik, not the REML log-lik.
        # To get the REML log-lik, we substract the last term in equation (48)
        # (also see the difference between equations (32-33) and (34-35) )
        if (i==j) {
          trace.term.G <- trace.term.G - 2 * sum(diag(solve(Q.matrix) %*% (Q.G.i.j)))
          trace.term.E <- trace.term.E - 2 * sum(diag(solve(Q.matrix) %*% (Q.E.i.j)))
        } else {
          trace.term.G <- trace.term.G - sum(diag(solve(Q.matrix) %*% (Q.G.i.j + Q.G.j.i)))
          trace.term.E <- trace.term.E - sum(diag(solve(Q.matrix) %*% (Q.E.i.j + Q.E.j.i)))
        }
        ###############

sort_snps           <- function(snp.list,
  ...
  #row.ind <- (nrow(w)-length(snp.list)):(nrow(w)-1)

  #rn      <- rownames(w)[row.ind]

  #pvalues.init <- w[row.ind, 4]

  #pvalues <- rep(NA, length(pvalues.init))

  pvalues <- rep(NA,length(snp.list))
  ...
  for (snp in snp.list) {
    ...
    pvalues[which(snp.list==snp)] <-   w[nrow(w)-1, 4]

  }

  result <- data.frame(snp=as.character(snp.list),pval=pvalues)

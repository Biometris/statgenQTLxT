...
The next step involves adding phenotypic data to the gData object just created. dropsPheno contains genotypic means for 28 trials. To be able to run a multi-environments GWAS analysis the data has to be added as a data.frame with "genotype" as first column and may only contain traits after that. Other columns need to be dropped.    
...
gDataDrops <- createGData(gData = gDataDrops, pheno = PhenoDat)
```
...
To get an idea of the contents of the data a summary of the gData object can be made. This will give an overview of the content of the map and markers and also print a summary per trait per trial (here only one).
```{r sumGData}
...
To demonstrate the options of the *codeMarkers* function, see the statgenGWAS vignette. 

## **Multi-trait GWAS**

The cleaned gData object can be used for performing multi-trait GWAS analysis. In the examples below the trait grain yield is used in multiple trials to demonstrate the options of the *runMultiTraitGwas* function. 
```{r mtg}
## Run multi-trait GWAS for traits 'grain.yield' in multiple trials.
GWASDrops <- runMultiTraitGwas(gData = gDataDropsDedup, covModel = "fa")
```
...
```{r qtlMtgNorm}
## Plot a qtl plot of GWAS DROPS for grain yield.
## Set significance threshold to 6 and do not normalize effect estimates.
plot(GWASDrops, plotType = "qtl", yThr = 6, normalize = FALSE)
```
...
```{r mtgChrSpec}
## Run multi-trait GWAS for grain yield in multiple trials.
...
```{r mtgSNPNR}
## Run multi-trait GWAS for for grain yield in multiple trials.
...
GWASDropsCommon <- runMultiTraitGwas(gData = gDataDropsDedup,
                                     covModel = "fa",
                                     estCom = TRUE)
head(GWASDropsCommon$GWAResult$`1`)
